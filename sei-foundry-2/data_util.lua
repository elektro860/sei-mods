...
function result.create_casting_recipe(params)
  ...
  if not (item or recipe or params.icons) then
    return
  end
  ...
  return {
    type = "recipe",
    name = "casting-" .. name,
    icons = (params.icons and #params.icons == 1 and result.sub_icons(params.icon or item.icon, params.icons[1]))
      or (params.icons and result.sub_icons(params.icon or item.icon, params.icons[1], {
