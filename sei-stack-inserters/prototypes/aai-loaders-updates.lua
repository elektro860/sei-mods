...
if
  settings.startup["aai-loaders-belt-stacking-mode"]
  and settings.startup["aai-loaders-belt-stacking-mode"].value ~= "off"
then
  return
end

if
  settings.startup["sei-add-stacking-to-aai-loaders"].value
  and not (settings.startup["aai-loaders-mode"].value == "graphics-only")
then
  ...
  if settings.startup["sei-loaders-wait-for-full-stack"].value then
    data.raw["utility-constants"]["default"].minimum_recipe_overload_multiplier =
